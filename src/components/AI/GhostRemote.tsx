...
export const GhostRemote: React.FC<GhostRemoteProps> = ({ onClose }) => {
    ...
    useEffect(() => {
        ...
        const peers = mesh.getConnectedPeerIds();
        if (peers.length > 0) {
            setWorkerId(peers[0]);
        }
    ...
    return (
        ...
        >
            ...
            <div className="relative w-full max-w-2xl bg-white border border-black/5 rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col h-[80vh]">
                {/* Header */}
                <div className="px-8 py-6 border-b border-black/[0.03] flex items-center justify-between bg-slate-50">
                    <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-2xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center">
                            <Smartphone className="w-5 h-5 text-blue-600" />
                        </div>
                        <div>
                            <h2 className="text-[14px] font-black uppercase tracking-[0.2em] text-slate-900 flex items-center gap-2">
                                The Sceptre <Zap className="w-3 h-3 text-blue-600 fill-blue-600" />
                            </h2>
                ...
                {/* Worker Status Shard */}
                <div className="px-8 py-4 bg-slate-50/50 border-b border-black/[0.03] flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2">
                            <Laptop className="w-3 h-3 text-emerald-600" />
                            <span className="text-[9px] font-black tracking-widest text-emerald-600 uppercase">Worker Online</span>
                        </div>
                        {tps > 0 && (
                            <div className="flex items-center gap-2">
                                <Cpu className="w-3 h-3 text-blue-600" />
                                <span className="text-[9px] font-mono text-blue-600">{tps} T/s</span>
                            </div>
                        )}
                    </div>
                    {isThinking && (
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] font-black tracking-widest text-blue-600 uppercase">Thinking...</span>
                        </div>
                    )}
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-4 bg-slate-50/30">
                    {error && (
                        <div className="p-4 rounded-2xl bg-rose-500/5 border border-rose-500/20 text-rose-500 text-[12px] font-mono">
                            ⚠️ GHOST_ERROR: {error}
                        </div>
                    )}
                    ...
                    {!stream && !isThinking && !error && (
                        <div className="h-full flex flex-col items-center justify-center opacity-30 gap-4">
                            ...
                            <p className="text-[11px] font-black uppercase tracking-[0.5em] text-center text-slate-400">
                                Awaiting Pulse<br />
                    ...
                    {stream && (
                        <div className="text-[15px] leading-relaxed text-slate-900 font-bold tracking-tight whitespace-pre-wrap selection:bg-blue-500/20">
                            {stream}
                ...
                {/* Pulse Input */}
                <div className="p-6 md:p-8 bg-slate-50 border-t border-black/[0.03]">
                    <div className="relative group">
                        <div className="absolute -inset-1 bg-gradient-to-r from-blue-500/20 to-blue-300/20 rounded-3xl blur-xl opacity-0 group-focus-within:opacity-100 transition-opacity duration-700" />
                        <div className="relative flex items-center gap-4 bg-white border border-black/5 rounded-2xl p-4 group-focus-within:border-blue-500/40 transition-all shadow-sm">
                            <input
                                value={input}
                                onChange={(e) => setInput(e.target.value)}
                                onKeyDown={(e) => e.key === "Enter" && handleSend()}
                                placeholder="Manifest a remote thought..."
                                className="flex-1 bg-transparent border-none outline-none text-[14px] text-slate-900 font-bold placeholder-slate-300"
                            />
                            <button
                                onClick={handleSend}
