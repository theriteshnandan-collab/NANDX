...
export class UsageService {
    ...
    private static getSupabase() {
        ...
        if (!serviceKey || !url) {
            console.warn("⚠️ SUPABASE_SERVICE_ROLE_KEY or URL missing. Usage tracking deactivated.");
            // Return a mock object to prevent crashes if called during build
            return {
                from: () => ({
                    select: () => ({ eq: () => ({ single: () => ({ data: null, error: null }), order: () => ({ limit: () => ({ data: [], error: null }) }), head: true, count: "exact" }) }),
                    upsert: () => ({ error: null }),
    ...
    static async checkCap(user_id: string, tool: string): Promise<CapCheckResult> {
        ...
        try {
            // Fetch user profile
            let { data: profile, error } = await this.getSupabase()
                .from("profiles")
            ...
            if (error || !profile) {
                ...
                const { data: newProfile } = await this.getSupabase()
                    .from("profiles")
                    .select("credits_balance, tier, daily_usage, last_usage_reset")
                    .eq("id", user_id)
                    .single();
    ...
    static async recordTransaction(user_id: string, action: string, cost: number) {
        const { error } = await this.getSupabase().from("usage_logs").insert({
            user_id,
            endpoint: action,
            method: "TRANSACTION",
            status_code: 200,
    ...
    static async getUserStats(user_id: string) {
        ...
        // Total Requests
        const { count: totalRequests } = await this.getSupabase()
            .from("usage_logs")
            .select("id", { count: "exact", head: true })
            .eq("user_id", user_id);

        // Success Rate (Last 100 requests)
        const { data: recentLogs } = await this.getSupabase()
            .from("usage_logs")
            .select("status_code")
            .eq("user_id", user_id)
            .order("created_at", { ascending: false })
